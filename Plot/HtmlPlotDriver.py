      ...
      <table class="logic-compare-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>候选线段端点极值替换模式。</td>
            <td>保留 <code>chan</code> 的特征序列确认主干，但在反向证据确认前，如果同方向有效笔端点出现更低底或更高顶，就把候选线段终点替换到该更极端端点。</td>
            <td>确认段的缺口和特征序列确认逻辑不变；替换只发生在本次确认窗口内，线段一旦落定后，不会再被更后面的行情回头重写。</td>
            <td>适合观察主跌/主升过程中，潜在线段是否应延伸到后续更低底或更高顶。</td>
            <td>输出通常会比 <code>chan</code> 线段更少、单段跨度更长；适合与 <code>chan</code> 对照使用。</td>
          </tr>
          <tr>
            <td><code>1+1</code></td>
